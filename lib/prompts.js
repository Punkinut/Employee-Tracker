...
const {printInfo, fetchInfo, budgetAdd } = read;
const { updateEx } = update;
...
// Placeholder variables that will later be defined and collect arrays from the database
let employees;
let roles;
let departments;
let managers;
...
const checkBudget = (depList) => {
    inquirer
        ...
        .then((res) => {
            const { deps } = res;
            budgetAdd(`SELECT department.name as department, role.salary FROM employee LEFT JOIN role ON employee.role_id=role.id LEFT JOIN department ON role.department_id=department.id LEFT JOIN employee manager ON employee.manager_id=manager.id WHERE department.name='${deps}'`)
            .then((data) => {
                console.log(`The total combined salaries used in this department is $${data}`)
            })
            .then(() => menuPrompts());
        })
};
...
const updateEmployeeManager = (employee, managy) => {
    inquirer
        ...
        .then((res) => {
            ...
            if (manager === 'None') {
                updateEx(employee, null, 'updateManager').then(() => menuPrompts());
            } else {
                managy.forEach((many) => {
                    if (many === manager) {
                        let index = managy.indexOf(many);
                        let newId = managerIds[index];
                        updateEx(employee, newId, 'updateManager').then(() => menuPrompts());
                    }
                })
            }
        })
...
const updateEmployeeRole = (employee, roley) => {
    inquirer
        ...
        .then((res) => {
            ...
            addition.then(() => {
                updateEx(employee, roleId, 'updateRole').then(() => menuPrompts());
            })
        })
};
