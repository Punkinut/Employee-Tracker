...
const readInfo = (input) => {
    const getDisc = new Read();
    if(input === 'role') {
        getDisc.fetchInfo('SELECT department.id, department.name as title, department.utilized_budget FROM department').then((discNames) => {
            addRole(discNames);
        })
    } else if (input === 'department') {
        getDisc.fetchInfo('SELECT * FROM role').then((discNames) => {
