        ...
        .then((res) => {
            ...
            switch (choice) {
                ...
                case 'View All Employees':
                    printInfo('SELECT employee.id, employee.first_name, employee.last_name, role.title, department.name as department, role.salary, employee.manager_id FROM employee INNER JOIN role  ON employee.role_id=role.id INNER JOIN department ON role.department_id=department.id').then(() => {
                        menuPrompts();
                    })
                    break;
                case 'Add Employee':
...
// Prints specific column names 
const readInfo = (input) => {
    if(input === 'role') {
